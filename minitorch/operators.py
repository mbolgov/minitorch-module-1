...
def lt(x: float, y: float) -> float:
    ...
    return 1.0 if x < y and not is_close(x, y) else 0.0
...
def is_close(x: float, y: float) -> float:
    "$f(x) = |x - y| < 1e-2$"
    return float(math.isclose(x, y, abs_tol=1e-2))
...
def sum(ls: Iterable[float]) -> float:
    ...
    return reduce(lambda x, acc: add(x, acc), 0.0)(ls)


def prod(ls: Iterable[float]) -> float:
    "Product of a list using `reduce` and `mul`."
    return reduce(lambda x, acc: mul(x, acc), 1.0)(ls)
